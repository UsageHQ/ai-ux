...
import ChatGPT from '@assets/chatgpt-progress.webp';
...
<div className='w-full grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-8'>
  <SampleUI
    src={Canva}
    appName= 'Canva'
    desc= "Canva show random progress bar to represent the progress. The finish time and what happen currently is unknown. This is a basic pattern to reduce the user's anxiety if the process takes a long time."
    productUrl= 'https://www.canva.com/'
  />
    <SampleUI
    src={ChatGPT}
    appName= 'ChatGPT'
    desc= "When talk to ChatGPT on mobile device, ChatGPT does not show the progress, they use an animation instead to relax the user. The user can see the AI is working on the task. This is a good pattern to reduce the user's anxiety."
