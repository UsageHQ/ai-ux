...
## Examples of Determinate Progress Indicators
...
<div className='w-full grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-8'>
  <SampleUI
    src={MidJourney}
    appName='Midjourney'
    desc= 'When generating images, Midjourney shows clear current progress. It tells users if prompt is start to precess or not and the current progress. When images start to be generated, images will be progressively render to the user.'
    productUrl='https://midjourney.com/'
  />
    <SampleUI
    src={Synthesia}
    appName='Synthesia'
    desc="Synthesia show detail progress by showing the rendering progress. The video's screen will be show and the script will be filled gradually. It helps users know exactly what is happening and how long it will take."
    productUrl='https://www.synthesia.io/'
  />
    <SampleUI
    src={ChatGPT}
    appName= 'ChatGPT'
    desc= "During process of generating text responses, ChatGPT show context message to let users know what happening under the hood. It helps clarify why certain processing steps are necessary and how they contribute to a more accurate or personalized outcome."
